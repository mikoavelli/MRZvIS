...
import numpy as np
from binary_logic import from_binary, inverse_binary, sum_binary, to_binary
...
def plot_ky_vs_n(
    ...
    if 1 not in final_xticks and n_max_plot_param >= 1:
        final_xticks = np.insert(final_xticks, 0, 1)
    if n_max_plot_param not in final_xticks and n_max_plot_param >= 1:
...
def plot_e_vs_n(
    ...
    final_xticks = final_xticks[
        (final_xticks >= 1) & (final_xticks <= n_max_plot_param + 1)
    ]  # +1 для видимости последней точки
    if len(final_xticks) > 0:
        plt.xticks(final_xticks)
    else:
        plt.xticks([1])
...
class Conveyer:
    ...
    @staticmethod
    def _check_values(
        list_dividend: list[str], list_divisor: list[str], num_of_bin_digits: int
        ...
        try:
            ...
            if num_of_bin_digits <= 0:
                raise MyException("Count (n) must be > 0")
            if len(dividends) <= 0:
                raise MyException("Number of tasks (r) must be > 0")
            if len(dividends) != len(divisors):
                raise MyException("Dividend/divisor list length mismatch")
    ...
    def _generate_conveyer(self) -> None:
        ...
        for i in range(self._count):
            self._conveyer[f"step{i + 1}"] = None
    ...
    def _generate_and_show_plots(self):
        ...
        plot_ky_vs_n(
            n_sim,
            r_sim,
            n_max_plot_param=n_plot_limit,
            r_max_curves_param=r_curves_limit,
        )
        plot_e_vs_n(
            n_sim,
            r_sim,
